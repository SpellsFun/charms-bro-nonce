...
double_sha256_persistent_kernel(
    ...
    // 载入outpoint到共享内存
    for (int i = threadIdx.x; i < outpoint_len && i < 80; i += blockDim.x) {
    ...
    // ILP展开因子 - 降低到2以减少寄存器压力
    const int ILP = 2;

    while (!(*done)) {
        // 原子获取下一个工作块 - 使用unsigned long long*类型转换
        ...
        for (uint64_t base = idx; base < end; base += blockDim.x * gridDim.x * ILP) {
            ...
            for (int ilp = 0; ilp < ILP; ilp++) {
                ...
                if (msg_len < 56) {
                    ...
                    // 手动大端转换
                    ((uint8_t*)W)[56] = (bit_len >> 56) & 0xff;
                    ((uint8_t*)W)[57] = (bit_len >> 48) & 0xff;
                    ((uint8_t*)W)[58] = (bit_len >> 40) & 0xff;
                    ((uint8_t*)W)[59] = (bit_len >> 32) & 0xff;
                    ((uint8_t*)W)[60] = (bit_len >> 24) & 0xff;
                    ((uint8_t*)W)[61] = (bit_len >> 16) & 0xff;
                    ((uint8_t*)W)[62] = (bit_len >> 8) & 0xff;
                    ((uint8_t*)W)[63] = bit_len & 0xff;
                }

                // 转换为大端
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    W[i] = cuda_bswap32(W[i]);
                }
                ...
                if (msg_len >= 56) {
                    memset(W, 0, sizeof(W));
                    uint64_t bit_len = msg_len * 8;
                    W[14] = cuda_bswap32((uint32_t)(bit_len >> 32));
                    W[15] = cuda_bswap32((uint32_t)(bit_len & 0xffffffff));
                    sha256_compress_words(W, H1);
                }
                ...
                #pragma unroll
                for (int i = 0; i < 8; i++) {
                    W[i] = cuda_bswap32(H1[i]);
                }
                W[8] = 0x80000000;  // padding
                W[15] = cuda_bswap32(256);  // 长度：256位
                ...
                #pragma unroll
                for (int i = 0; i < 8; i++) {
                    uint32_t word = cuda_bswap32(H2[i]);
                    if (word == 0) {
                        lz += 32;
                    } else {
                        lz += __clz(word);
    ...
    if (local_best_lz > 0) {
        ...
        // 如果我们有最佳值，更新nonce - 使用unsigned long long*
